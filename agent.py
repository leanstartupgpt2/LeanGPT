...
def get_openai_client():
    """Get OpenAI client with the API key from config"""
    # Get API key from database config, Streamlit secrets, or environment variable
    config = db_manager.get_config()
    api_key = config.get("openai_api_key", "")
    
    # If not found in database, try Streamlit secrets
...
def get_agent_response(user_input, system_prompt, chat_history):
    ...
    # Check Streamlit secrets if not in database
    if not api_key and hasattr(st, 'secrets') and 'openai' in st.secrets:
        api_key = st.secrets.openai.api_key
    
    # Check environment variables if not in Streamlit secrets
    if not api_key:
        api_key = os.environ.get("OPENAI_API_KEY", "")
